...
const MainMenu: FC = () => {
  return (
    <>
      <div
        className="min-h-screen max-h-screen  menu-bg text-white
     flex flex-col  py-10 relative z-1 overflow-y-hidden"
      >
        <BackgroundAnimation className="absolute inset-0 -top-15 overflow-x-hidden mx-4 " />
        ...
        <main className="flex flex-col gap-[15vh] items-center">
          ...
          <Image
            src={logo}
            alt="FRAGMENTS"
            className="px-3 sm:px-10 max-w-[90vw]  h-auto z-100"
          />
          <BottomButtons />
        </main>
      </div>
  ...
  return (
    <div className="flex flex-col gap-4 w-full z-100">
      ...
      <MenuButton
        ...
        notificationCount={2}
        className="w-[45vw] sm:w-3/4 md:w-2/3 lg:w-1/2 xl:w-1/3 max-w-sm rounded-r-2xl"
      ...
      <MenuButton
        ...
        rightIcon={powers}
        gradientClass="orange-gradient"
        className="w-[35vw] sm:w-1/2 md:w-1/3 max-w-xs rounded-r-2xl"
      />
    </div>
  );
};
